...
namespace Paradise.Realtime.Core {
	public class LoopScheduler : ILoopScheduler, IDisposable {
		...
		public LoopScheduler(float tickRate) {
			...
			_thread = new Thread(DoScheduling) {
				Name = "LoopScheduler-thread-" + Interlocked.Increment(ref s_id)
			};
		...
		private void PauseInternal() => _pauseHandle.Reset();

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void ResumeInternal() => _pauseHandle.Set();
