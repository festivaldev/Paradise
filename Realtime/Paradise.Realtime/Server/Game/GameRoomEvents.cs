	...
	public class GameRoomEvents : BaseEventSender {
		...
		public override SendResult SendEvent(byte opCode, MemoryStream bytes) {
			if (Enum.IsDefined(typeof(IGameRoomEventsType), (int)opCode)) {
				switch ((IGameRoomEventsType)opCode) {
					case IGameRoomEventsType.AllPlayerPositions:
					case IGameRoomEventsType.AllPlayerDeltas:
						break;
					default:
						Log.Debug($"GameRoomEvents.SendEvent: {(IGameRoomEventsType)opCode}");
						break;
				}
			}
