...
namespace Paradise.Realtime.Server.Game {
	public partial class GamePeer {
		public class EventSender : BaseEventSender {
			private static readonly ILog Log = LogManager.GetLogger(nameof(GamePeer.EventSender));

			public BaseGameRoom.EventSender GameEventSender { get; private set; }

			public EventSender(BasePeer peer) : base(peer) {
				GameEventSender = new BaseGameRoom.EventSender(peer);
			}
			...
			public override SendResult SendEvent(byte opCode, MemoryStream bytes) {
				if (Enum.IsDefined(typeof(IGamePeerEventsType), (int)opCode)) {
					Log.Debug($"GamePeer.EventSender::SendEvent: {(IGamePeerEventsType)opCode}");
				}

				return base.SendEvent(opCode, bytes);
			}

			#region Implementation of IGamePeerEventsType
			/// <summary>
