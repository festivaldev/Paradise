...
namespace Paradise.WebServices.LegacyServices {
	internal class GameSessionManager {
		...
		public GameSession FindOrCreateSession(PublicProfileView profile, string machineId, UserAccount userAccount) {
			if (TryGetValue(profile.Cmid, out var session)) {
				return session;
			}

			session = new GameSession {
				SessionId = CreateSessionId(profile.Cmid, ((DateTimeOffset)DateTime.UtcNow).ToUnixTimeMilliseconds()),
				Cmid = profile.Cmid,
				MachineId = machineId
			};
