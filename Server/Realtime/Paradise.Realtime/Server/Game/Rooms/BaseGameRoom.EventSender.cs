...
namespace Paradise.Realtime.Server.Game {
	public partial class BaseGameRoom {
		public class EventSender : BaseEventSender {
			private static readonly ILog Log = LogManager.GetLogger(nameof(BaseGameRoom.EventSender));

			public EventSender(BasePeer peer) : base(peer) { }

			public override SendResult SendEvent(byte opCode, MemoryStream bytes) {
				if (Enum.IsDefined(typeof(IGameRoomEventsType), (int)opCode)) {
					Log.Debug($"BaseGameRoom.EventSender::SendEvent: {(IGameRoomEventsType)opCode}");
				}

				return base.SendEvent(opCode, bytes);
			}

			#region Implementation of IGameRoomEventsType
